...
prob3(N) ->
    sieve:start(),
    {Time, List} = timer:tc(?MODULE, prob3, [N, sieve:first_prime(), []]),
    ?debugFmt("Prob3 time: ~p", [Time/1000000]),
    lists:sort(List).
...
prob4(O, I, H, E) ->
    Product = O*I,
    case euler_util:is_palindrome(Product) of
        true ->
...
prob16() ->
    lists:foldl(fun(C, A) -> C+A-$0 end, 0, integer_to_list(euler_util:pow(2, 1000))).

%%% Find the sum of the digits in the number 100!
prob20() ->
    lists:foldl(fun(C, A) -> C+A-$0 end, 0, integer_to_list(euler_util:factorial(100))).
